...
import { initStars, updateStars } from '../entities/background';
import { createPlayerShip, updatePlayerShip } from '../entities/player';
import { spawnAsteroid, updateAsteroids } from '../entities/asteroid';
import { fireBullets, updateBullets } from '../entities/bullet';
import { spawnPowerup, updatePowerups } from '../entities/powerup';
import { updateExplosions, createExplosion } from '../entities/effects';
...
export class GameEngine {
  ...
  private difficulty: 'recruit' | 'soldier' | 'veteran' | 'hero';

  constructor(canvas: HTMLCanvasElement, difficulty: 'recruit' | 'soldier' | 'veteran' | 'hero' = 'soldier') {
    this.canvas = canvas;
...
export function updateGameState(
  ...
  let newBulletCooldown = Math.max(0, spawnTimers.bulletCooldown - deltaTime * 1000);
  ...
  // Fire bullets (if spacebar is pressed and cooldown is ready)
  // Max 2 bullets per second = 500ms cooldown
  let newBullets = [...state.bullets];
  if (keysPressed[' '] && newBulletCooldown === 0) {
    ...
    newBulletCooldown = 500; // 500ms = 0.5 seconds = 2 bullets per second
  }
  ...
  for (const asteroid of finalAsteroids) {
    if (checkPhysicsCollision(finalPlayer, asteroid)) {
      ...
      if (finalPlayer.shieldLevel > 0) {
        ...
        // Player takes damage (modified by difficulty)
        finalPlayer.health -= asteroid.damage * damageMultiplier;
        asteroidsToRemove.add(asteroid.id);
  ...
  return {
    ...
    newTimers: {
      ...
      bulletCooldown: newBulletCooldown,
    },
  };
}
